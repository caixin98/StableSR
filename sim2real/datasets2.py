...
class PairedCaptureDataset(data.Dataset):
    def __init__(self, sim_capture_path, real_capture_path, transform=transform, return_name=False, data_num = None):
        ...
        #TODO: check if sim and real capture file name match
        real_captures_name = [real_capture_name.split(".")[0] for real_capture_name in real_captures]
        
        for sim_capture in sim_captures:
            if sim_capture.split(".")[0] not in real_captures_name:
                print(sim_capture)
                raise ValueError
            self.paired_captures.append((sim_capture, sim_capture.split(".")[0] + ".jpg"))
