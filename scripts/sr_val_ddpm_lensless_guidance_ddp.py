...
# the generated images by ddp are kind of different from the single gpu version
# therefore we prefer the single gpu version
...
class ImageDataset(Dataset):
	def __init__(self, init_img_dir, outpath, origin_img_dir=None, transform=None, ):
	...
	def __getitem__(self, idx):
		...
		if self.origin_img_dir is not None:
			...
			return image, lensless_image, img_name
...
def ddp_main(rank, opt, world_size):
	...
	image_dataloader = DataLoader(image_dataset, batch_size=batch_size, shuffle=False, num_workers=batch_size // 2, pin_memory=True, sampler=sampler)
	...
	with torch.no_grad():
		with precision_scope("cuda"):
			with model.ema_scope():
				tic = time.time()
				count = 0
				for init_image, gt_image, img_names in tqdm(image_dataloader):
